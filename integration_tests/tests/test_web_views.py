...
from integration_tests.factories import (
    BrandFactory,
    EnterpriseFactory,
    TripFactory,
    VehicleFactory,
...
class TestVehicleViews:
    ...
    def test_vehicle_update_get(self, web_client):
        ...
        url = reverse("vehicles:update", kwargs={"pk": vehicle.pk})
        response = web_client.get(url)
    ...
    def test_vehicle_delete_get(self, web_client):
        ...
        url = reverse("vehicles:delete", kwargs={"pk": vehicle.pk})
        response = web_client.get(url)
...
class TestEnterpriseViews:
    ...
    def test_enterprise_vehicles_list(self, web_client):
        ...
        url = reverse("vehicles:list")
        url = f"{url}?enterprise_id={enterprise.pk}"
        response = web_client.get(url)

        assert response.status_code == 200
        assert "ENT123" in response.content.decode()
        assert enterprise.name in response.content.decode()
...
class TestManagerPermissions:
    ...
    def test_manager_cannot_access_other_enterprise_vehicle_detail(
        ...
        url = reverse("vehicles:detail", kwargs={"pk": other_vehicle.pk})
        response = web_client.get(url)
...
class TestExportViews:
    ...
    def test_vehicle_export_json(self, web_client):
        ...
        url = reverse("vehicles:export") + "?export_format=json"
        response = web_client.get(url)
    ...
    def test_enterprise_export(self, web_client):
        ...
        url = reverse("enterprises:export", kwargs={"pk": enterprise.pk})
        response = web_client.get(url)
...
class TestTripMapView:
    ...
    def test_trip_map_post(self, web_client):
        ...
        url = reverse("tracking:trips_map")
