...
DJANGO_API_URL = f"http://{DJANGO_HOST}/api/"
...
async def set_jwt_token(username, password, user_id):
    url = f"{DJANGO_API_URL}accounts/token/"
...
async def refresh_jwt_token(user_id):
    url = f"{DJANGO_API_URL}accounts/token/refresh/"
...
def create_consumer(max_retries=1000, delay=5):
...
def consume_speed_alerts(application):
    """
    Kafka consumer для speed alerts
    """
    try:
